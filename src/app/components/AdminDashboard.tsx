...
export default function AdminDashboard() {
  ...
  const baseStats = [
    ...
    },
    {
      ...
      icon: Clock,
      color: 'bg-orange-500',
      // clicking Pendientes opens the approval panel
      ...
      <div className="space-y-6">
        ...
        {stats.pendientes > 0 && (
          <button
            ...
            className="w-full bg-orange-50 border border-orange-200 rounded-xl p-4 flex items-center gap-3 hover:bg-orange-100 transition-colors text-left"
          >
            <AlertCircle className="w-5 h-5 text-orange-500 flex-shrink-0" />
            <div className="flex-1">
              <p className="text-sm font-medium text-orange-800">
                {stats.pendientes} cita{stats.pendientes !== 1 ? 's' : ''} pendiente{stats.pendientes !== 1 ? 's' : ''} de aprobación
              </p>
              <p className="text-xs text-orange-600 mt-0.5">
                Haz clic aquí para aprobar o rechazar
              </p>
            </div>
            ...
            return (
              <div
                ...
                className={`bg-white rounded-xl shadow-sm p-6 border border-gray-100 transition-all ${
                  isClickable
                    ? 'cursor-pointer hover:shadow-md hover:border-orange-200 hover:bg-orange-50 group'
                    : ''
                }`}
              >
                <div className="flex items-start justify-between">
                  <div>
                    <p className="text-sm text-gray-500 mb-1 flex items-center gap-1">
                      {stat.label}
                      {isClickable && (
                        <span className="text-xs text-orange-500 font-medium">(ver)</span>
                      )}
                    </p>
                    <h3 className={`text-3xl font-bold ${isClickable ? 'text-orange-600' : 'text-gray-900'}`}>
                      {statsLoading
                        ? <span className="inline-block w-10 h-8 bg-gray-100 rounded animate-pulse" />
                        : stat.value}
                    </h3>
                  </div>
                  <div className={`${stat.color} rounded-lg p-3 ${isClickable ? 'group-hover:scale-110 transition-transform' : ''}`}>
                    <Icon className="w-6 h-6 text-white" />
                  </div>
                </div>
                {isClickable && !statsLoading && stat.value > 0 && (
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <div className="text-right">
                        <p className="font-medium text-gray-900">
                          {esHoy ? <span className="text-indigo-600 font-semibold">Hoy</span> : fecha} · {hora}
                        </p>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <span className="font-semibold text-gray-900">{statsLoading ? '—' : stats.confirmadas}</span>
                </div>
                <div
                  className={`flex items-center justify-between ${stats.pendientes > 0 ? 'cursor-pointer hover:bg-orange-50 rounded-lg px-2 py-1 -mx-2 transition-colors' : ''}`}
                  onClick={stats.pendientes > 0 ? openNotifications : undefined}
                >
                  <div className="flex items-center gap-2 text-sm text-gray-600">
                    <Clock className="w-4 h-4 text-yellow-500" />
                    Pendientes
                    {stats.pendientes > 0 && <span className="text-xs text-orange-500 font-medium">(aprobar)</span>}
                  </div>
                  <span className={`font-semibold ${stats.pendientes > 0 ? 'text-orange-600' : 'text-gray-900'}`}>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className="font-semibold text-gray-900">{statsLoading ? '—' : stats.completadas}</span>
                </div>
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2 text-sm text-gray-600">
                  ...
                  <span className="font-semibold text-gray-900">{statsLoading ? '—' : stats.canceladas}</span>
                </div>
            ...
            <div>
              ...
              <p className="text-indigo-100">
                ...
                {stats.pendientes > 0 && (
                  <button
                    onClick={openNotifications}
                    className="ml-2 font-semibold text-yellow-300 underline hover:text-yellow-100 transition-colors"
                  >
