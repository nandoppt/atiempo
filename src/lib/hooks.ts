...
async function enrichCitasWithRelations<T extends Partial<Cita>>(citas: T[]) {
  if (!citas?.length) return citas

  const clienteIds = Array.from(new Set(citas.map(c => c.cliente_id).filter(Boolean)))
  const servicioIds = Array.from(new Set(citas.map(c => c.servicio_id).filter(Boolean)))

  const [clientesResult, serviciosResult] = await Promise.all([
    clienteIds.length ? supabase.from('clientes').select('*').in('id', clienteIds) : Promise.resolve({ data: [] as Cliente[], error: null }),
    servicioIds.length ? supabase.from('servicios').select('*').in('id', servicioIds) : Promise.resolve({ data: [] as Servicio[], error: null }),
  ])

  const clientesById = new Map<string, Cliente>()
  const serviciosById = new Map<string, Servicio>()

  clientesResult.data?.forEach(cliente => { if (cliente?.id) clientesById.set(cliente.id, cliente) })
  serviciosResult.data?.forEach(servicio => { if (servicio?.id) serviciosById.set(servicio.id, servicio) })

  return citas.map(cita => ({
    ...cita,
    clientes: cita.cliente_id ? clientesById.get(cita.cliente_id) ?? undefined : undefined,
    servicios: cita.servicio_id ? serviciosById.get(cita.servicio_id) ?? undefined : undefined,
  }))
}
...
export function useCitas(filterEstado?: CitaEstado | 'all') {
  ...
  const fetchCitas = useCallback(async () => {
    ...
    if (error) {
      ...
      setCitas(await enrichCitasWithRelations(data ?? []))
    }
    setLoading(false)
  }, [filterEstado])
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(`citas-admin-changes-${Math.random().toString(36).slice(2)}`)
...
export function useCitasCliente(clienteId: string | undefined) {
  ...
  const fetchCitas = useCallback(async () => {
    ...
    try {
      ...
      setError((err as Error).message || 'Error inesperado al obtener citas')
      setCitas([])
    }
    setLoading(false)
  }, [clienteId])
...
export function useHorariosDisponibles(fecha: string, duracionMinutos: number) {
  ...
  const fetchHorarios = useCallback(async () => {
    ...
    const { data: citasDelDia } = await supabase
      .from('citas')
      .select('fecha_hora_inicio, servicio_id')
      .gte('fecha_hora_inicio', startOfDay)
      .lte('fecha_hora_inicio', endOfDay)
      .neq('estado', 'cancelada')

    const citasConRelaciones = await enrichCitasWithRelations(citasDelDia ?? [])

    // Build occupied intervals
    const ocupados: Array<{ inicio: Date; fin: Date }> = []
    citasConRelaciones.forEach((c: any) => {
...
export function useCitasPendientes() {
  ...
  // Cambiamos el nombre de "fetch" a "fetchPendientes"
  const fetchPendientes = useCallback(async () => {
    ...
    setPendientes(await enrichCitasWithRelations(data ?? []))
    setLoading(false)
  }, [])
  ...
  useEffect(() => {
    fetchPendientes()
    const channel = supabase
      .channel(`pendientes-changes-${Math.random().toString(36).slice(2)}`)
      .on('postgres_changes', { event: '*', schema: 'public', table: 'citas' }, fetchPendientes)
      .subscribe()
    return () => { supabase.removeChannel(channel) }
  }, [fetchPendientes]) // Actualizamos la dependencia aquí también

  return { pendientes, refetch: fetchPendientes, loading }
}
...
export function useNotificacionesCliente(clienteId: string | undefined) {
  ...
  useEffect(() => {
    ...
    if (!clienteId) return
    const channel = supabase
      .channel(`notif-cliente-${clienteId}-${Math.random().toString(36).slice(2)}`)
      .on('postgres_changes', {
        event: '*', schema: 'public', table: 'citas',
        filter: `cliente_id=eq.${clienteId}`,
      }, fetchNotificaciones)
...
export async function updateEstadoCita(id: string, estado: CitaEstado) {
  console.log(`[updateEstadoCita] Updating cita ${id} to estado: ${estado}`)
  try {
    // First, verify the cita exists
    ...
    if (fetchError || !existing) {
      console.error(`[updateEstadoCita] Cita not found:`, fetchError || 'No data')
      return { error: fetchError || new Error('Cita no encontrada') }
    }
    
    console.log(`[updateEstadoCita] Found cita with current estado:`, existing.estado)
    ...
    if (error) {
      console.error(`[updateEstadoCita] Error updating cita:`, error)
    } else {
      console.log(`[updateEstadoCita] Success! Updated data:`, data)
    }
    return { error, data }
  } catch (e) {
    console.error(`[updateEstadoCita] Exception:`, e)
